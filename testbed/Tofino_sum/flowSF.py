...
s_conf = open("range.txt")
s = []
r_conf=open("ggs.txt")
r=[]
for t in s_conf:
    d = t.split()
    s.append((int(d[0]), int(d[1]),int(d[2]),int(d[3])))   
for t in r_conf:
    d = t.split()
    r.append((int(d[0]), int(d[1]),int(d[2])))  	
...
def connect():
    ...
    for kv in s:
        ...
        try:
            client.offset_table_delete_by_match_spec(sess_hdl, dev_tgt, match_spec)
        except:
            pass
        client.offset_table_add_with_os(sess_hdl, dev_tgt, match_spec, priority,action_spec)
    print "Port add done."
