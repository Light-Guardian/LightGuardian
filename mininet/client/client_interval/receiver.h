...
struct Receiver : Host {
	...
	void receive(int maxp) {
		for (int recvp = 0; recvp < maxp || maxp < 0; recvp++) {
			...
			if (com_header.flag_header.exists_fg == 12 || com_header.flag_header.exists_fg == 8) {
	...
	void betoh_header(COM_Header &com_header) {
		...
		com_header.mih.fgment_id = be32toh(com_header.mih.fgment_id);

		com_header.mih.timestamp = be32toh(com_header.mih.timestamp);
