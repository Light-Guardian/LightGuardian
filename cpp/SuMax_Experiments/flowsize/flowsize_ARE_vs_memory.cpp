...
void calcu(ofstream & stlog){
    ...
    rep2(i, 0, are_num){
        ...
        rep2(j, 0, Lev_Num){
            ...
            size_cnt += sketch->query_lev(keyvalue[i].first, j);
            real_size += GT[keyvalue[i].first][j];
        ...
        size_are_sum += (double)ABS(size_cnt-real_size)/(double)real_size;
        wmre += wmre_tmp1/wmre_tmp2;
    }
    wmre /= (double) are_num;
    size_are_sum /= (double) are_num;
    ...
    stlog << Buck_Num_PerRow*4*3/1024 << ',' << size_are_sum << ',';
