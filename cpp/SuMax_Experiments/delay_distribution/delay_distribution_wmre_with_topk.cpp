...
int main(int argc, char *argv[]){
    ...
    topkthres = 1000;
        ...
        char outpath[100] = "./mixed_delay_wmre_vs_topk";
        //strcat(outpath,to_ssssstring(Row_Num).c_str());
        strcat(outpath, ".csv");
        ofstream stlog(outpath, std::ios::out | std::ios::trunc);
        
        stlog << "topk, acu wmre, cm wmre, cu wmre" << endl;
        Buck_Num_PerRow = 65536;
        int TOPK_ARRAY[6] = {10, 40, 160, 640, 2560, 10240};
        
        rep2(i, 0, 6){
            topkthres = TOPK_ARRAY[i];
            calcu(stlog);
        }
        stlog.close();
