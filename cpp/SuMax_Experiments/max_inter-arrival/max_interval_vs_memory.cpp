...
void ave(ofstream & stlog){
    ...
    stlog << Buck_Num_PerRow*Row_Num*8*2/1024 << ',' << arecnt << endl;
    return;
}

int main(){
    ofstream stlog("./row3_top1000_maxinterval_memory.csv", std::ios::out | std::ios::trunc);
    double basic_buck_num = 8192;
    Row_Num = 3;
    stlog << "memory(KB), ARE" << endl;
    topkthres = 1000;

    while(basic_buck_num <= 65536*4){
